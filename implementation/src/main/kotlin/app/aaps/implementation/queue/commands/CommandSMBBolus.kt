...
class CommandSMBBolus(
    ...
    override fun execute() {
        ...
        if (lastBolusTime != 0L && lastBolusTime + T.secs(30).msecs() > dateUtil.now()) {
            aapsLogger.debug(LTag.PUMPQUEUE, "SMB requested but still in 30 seconds interval")
            r = PumpEnactResult(injector).enacted(false).success(false).comment("SMB requested but still in 30 seconds interval")
